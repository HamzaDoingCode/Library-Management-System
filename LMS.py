...
class Book:
    ...
    def __init__(self, ID, Title, author):
        ...
        self.__availability = True # all the books are available by defaulf
    ...
    def borrow_book(self):
        if self.__availability:
            self.__availability = False # Marks its absence as it is going to be borrowed
        else:
...
class Student(Member): # Rule: Student can only borrow three books at a time
    def __init__(self, MemberID, Name, library):
        ...
        self.__bookQuantity = 0 # Number of borrowed books
...
class Library:
    ...
    def addBook(self, book): # Object of book passed as argument
        ...
        # Dictionary of book updates by writing id as key and object as value
    ...
    def addMember(self, mem):
        ...
        # Dictionary of members updates by writing id as key and object as value
    ...
    def borrow_book(self, person, bookID): # person is the object of class member/student/teacher
        if bookID not in self.books.keys(): # Checks that if the book of given ID exist in the library or not
            ...
            return
        if self.books[bookID].is_available(): # Checks that if, the book of given id is available
            ...
            person.borrowed_books[bookID] = self.books[bookID]  # append the dictionary of individual member, with ID as a key and book as a value
        else:
            ...
            # self is an obj of member, variable library also cotains the obj of Library, books[ID] corresponds to equivalent object of book in the dictionary of books, then get title from that object
...
def add_Member(library):
    ...
    # Passing object of class Member to the method of class library
...
def file_of_readers_records(library):
    ...
    try:
        with open("readers.txt", 'r') as file:
            content = file.read()
            if (content == ""): # Checks whether file is empty OR some data is stored.
                isEmpty = True
    # Closing read mode.
    except FileNotFoundError:
        with open("readers.txt", 'w') as file: # Creates the file if it does not exist
            isEmpty = True # If file does not exist, treat it as empty

    with open("readers.txt", "a") as f:
        if (isEmpty):
...
def file_of_books_records(library):
    ...
    # Closing read mode.
        ...
        with open("books.txt", 'w') as file: # Creates the file if it does not exist
            isEmpty = True # If file does not exist, treat it as empty
